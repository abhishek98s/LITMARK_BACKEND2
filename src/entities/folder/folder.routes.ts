...
router.get('/', getAllFolders).post('/', joiValidationMiddleware(folderSchema), upload.single('litmark_image'), verifyToken, postFolders)
router.patch('/:id', upload.single('litmark_image'), joiValidationMiddleware(folderSchema), verifyToken, patchFolders).delete('/:id', deleteFolders)
